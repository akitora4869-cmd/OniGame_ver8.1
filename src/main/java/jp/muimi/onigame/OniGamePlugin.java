...
public final class OniGamePlugin extends JavaPlugin implements Listener, CommandExecutor, TabCompleter {
    ...
    private void randomizeTagSpShops(){List<String> pool=new ArrayList<>(List.of("skill:SPRINT:4:2","skill:INVISIBLE:6:1","skill:SMOKE:5:2","skill:BLINK:7:1","skill:ECHO:6:2","skill:HEAL:6:1","skill:CLAIRVOYANCE:5:1","skill:SEALING_CIRCLE:8:1","skill:RESONANCE:7:1","skill:SUBSTITUTE:8:1","skill:DESPERATE_RUN:9:1","item:HEAL:3:1","item:FLARE:5:1"));Random r=new Random();for(String raw:getConfig().getStringList("tag-sp.shop-blocks")){List<String> pick=new ArrayList<>(pool);Collections.shuffle(pick,r);int count=Math.min(pick.size(),4+r.nextInt(3));getConfig().set("tag-sp.shop-products."+shopKey(raw),new ArrayList<>(pick.subList(0,count)));}saveConfig();}
    ...
    private void startTag(CommandSender starter,boolean force){
        ...
        resetRuntime();oni=null;oniTeam.clear();players.clear();randomizeTagSpShops();oniType=selectedOniType!=null&&!isOniTypeLocked(selectedOniType)?selectedOniType:firstUnlockedOni();List<UUID> ids=new ArrayList<>(participants);Collections.shuffle(ids);
